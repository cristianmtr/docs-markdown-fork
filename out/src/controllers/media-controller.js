    ...
    dir.files(folderPath, (err, files) => {
        ...
        selectionPick.then((qpSelection) => {
            if (!qpSelection) {
                ...
                var relativePath = encodeURI(
                    path.relative(activeFileDir, path.join(qpSelection.description, qpSelection.label).split("\\").join("\\\\"))
                    );
                ...
                if (isArt && altText) {
                    if (altText.length > 70) {
                        ...
                        result = utility_1.internalLinkBuilder(isArt, relativePath, altText);
                    }
                }
                else if (isArt && altText === "") {
                    result = utility_1.internalLinkBuilder(isArt, relativePath, altTextFileName);
                }
                else if (!isArt) {
                    result = utility_1.internalLinkBuilder(isArt, relativePath, selectedText);
                }
                if (os.type() === "Darwin") {
